...
const startServer = async (): Promise<void> => {
    try {
        ...
        io.on('connection', (socket: Socket) => {
            ...
            socket.on('submit_meme', (data: { gameId: string, captions: string[] }, callback: (response: any) => void) => {
                try {
                    console.log('Submit meme request:', { gameId: data.gameId, userId: socket.data.userId, captions: data.captions });

                    const result = gamesService.submitMeme(data.gameId, socket.data.userId.toString(), data.captions);
            ...
            socket.on('submit_vote', (data: { gameId: string, submissionPlayerId: string }, callback: (response: any) => void) => {
                try {
                    const result = gamesService.submitVote(data.gameId, socket.data.userId.toString(), data.submissionPlayerId);
                    if (result.success && result.game) {
                        const game = result.game;

                        // Notify all players about the vote
                        io.to(`game:${data.gameId}`).emit('vote_submitted', {
                            playerId: socket.data.userId,
                            username: socket.data.username,
                            round: game.round,
                            currentRound: game.currentRound
            ...
            socket.on('start_game', (data: { gameId: string }, callback: (response: any) => void) => {
                try {
                    ...
                    game.status = 'playing';
                    gamesService.startNewRound(game.id);
